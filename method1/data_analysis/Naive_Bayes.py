...
# Re-define the function to compute various scores for each fold for the new model
def compute_scores_per_fold(cv, X, y, model, threshold=0.5):
    ...
    file_name = 'NB_folds_' + root_name
    e.write_to_excel(file_name,all_folds)

    return mean_scores


# Compute average scores using Gaussian Naive Bayes and 10-fold cross-validation
mean_scores_nb = compute_scores_per_fold(cv_stratified, X, y, GaussianNB())
main_file_name = 'results_' + root_name
e.update_excel(main_file_name, "NB", mean_scores_nb)
